help([[
This module loads Samui Browser v1.0.1.
]])
...
if (mode() == "load") then
    LmodMessage("Loading LIBD SLURM module for samui/1.0.1")
elseif (mode() == "unload") then
    LmodMessage("Unloading LIBD SLURM module for samui/1.0.1")
end

-- Directly activate or deactivate the conda environment
always_load("conda/3-24.3.0")
execute {cmd="conda activate /jhpce/shared/libd/core/samui/1.0.1/samui_env", modeA={"load"}}
execute {cmd="conda deactivate", modeA={"unload"}}

-- Seems to be necessary for finding python scripts from the Samui repo
append_path('PYTHONPATH', '/jhpce/shared/libd/core/samui/1.0.1/samui')
